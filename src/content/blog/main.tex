...
Au final, avec la représentation sinusoidale proposée par Vaswani, \textbf{le produit scalaire entre deux encodages positionnels dépend uniquement de la différence $p-q$ et s’exprime comme une somme de cosinus à différentes fréquences, formant une signature multi-échelle de la distance relative}.


Malheureusement, cette dépendance claire à $p-q$ ne va pas être exploitée pleinement par les mécanismes d'attention des premiers modèles Transformer. En effet, dans ces modèles, les embeddings positionnels sont ajoutés aux embeddings de contenu :
...
\[
\langle f_q(x_q, q), f_k(x_p, p) \rangle = g(x_q, x_p, p - q)
\]
...
\parbox{6cm}{
\centering
position injectée dans les représentations de tokens\\
via un ajout direct aux embeddings
}
&
\parbox{6cm}{
\centering
position injectée via une rotation dépendant de la position
}
\\
\parbox{6cm}{
\centering
dépendance implicite au relatif
}
&
\parbox{6cm}{
\centering
dépendance algébrique explicite
}
\end{array}
\]
...
\text{dépendance à } (p-q)\ \text{indirecte}
&
\text{dépendance à } (p-q)\ \text{imposée}
\end{array}
\]
...
\begin{align}
z_k(p)\overline{z_k(q)}
&= e^{i\omega_k p} e^{-i\omega_k q} \\
&= e^{i\omega_k(p-q)}
\end{align}

\[
\Re(z_k(p)\overline{z_k(q)}) = \cos(\omega_k(p-q))
\]
...
\[
Q_p \cdot K_q \approx g(c_p, c_q, p-q)
\]
